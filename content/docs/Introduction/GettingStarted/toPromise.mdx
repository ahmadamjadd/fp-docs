...
### For `Option<T>`:
- `Option.Some(value)` → Resolves to `value` (type `T`)
- `Option.None` → Resolves to `null` (type `null`)

### For `Result<T, E>`:
- `Result.Ok(value)` → Resolves to `value` (type `T`)
- `Result.Err(error)` → Rejects with `error` (type `E`)
...
async function processPayment(order: Order): Promise<Result<Order, string>> {
  ...
  console.log(`💳 Payment processed: $${order.total.toFixed(2)}`);
  return Result.Ok(order);
}
...
async function processOrderAsync(order: Order): Promise<Result<string, string[]>> {
  console.log(`🚀 Starting async processing for order ${order.id}...`);
...
async function handleOrderProcessing() {
  ...
  Result.match(result, {
    Ok: (confirmation) => {
      console.log("✅ Async order processed successfully!");
      console.log(confirmation);
      console.log(`⏱️  Total processing time: ${duration}ms`);
    },
    Err: (errors) => {
      console.error("❌ Async order processing failed:");
...
/* Successful output:
🚀 Starting async processing for order ORD-ASYNC-001...
🔒 Reserved inventory for order ORD-ASYNC-001
💳 Payment processed: $80.00
📋 Order ORD-ASYNC-001 status updated
✅ Async order processed successfully!
